...
import createSchema from './migrate-cloud-volume.schema';
import miqRedirectBack from '../../helpers/miq-redirect-back';

const MigrateCloudVolumeForm = ({ recordId }) => {
  ...
  useEffect(() => {
    if (isLoading) {
      API.options(`/api/cloud_volumes/${recordId}?option_action=migrate`)
        .then(loadSchema());
    }
  });
  ...
  const onSubmit = (values) => {
    ...
    request.then(() => {
      const message = sprintf(
        __('Migrating of Cloud Volume has been successfully queued.'));
  ...
  const onCancel = () => {
    miqSparkleOn();
    const message = sprintf(
      __('Migrating Cloud Volume was cancelled by the user.'));
  ...
  return !isLoading && (
    <div className="tasks-form">
      <MiqFormRenderer
        ...
        buttonsLabels={{ submitLabel: __('Migrate')}}
      />
    </div>
  );
};
...
MigrateCloudVolumeForm.propTypes = {
  recordId: PropTypes.string,
};
MigrateCloudVolumeForm.defaultProps = {
  recordId: undefined,
};
...
export default MigrateCloudVolumeForm;
